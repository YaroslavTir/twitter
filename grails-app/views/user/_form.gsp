...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'profile', 'error')} ">
    ...
    <g:select id="profile" name="profile.id" from="${com.grailsinaction.Profile.list()}" optionKey="id"
              value="${userInstance?.profile?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'following', 'error')} ">
    ...
    <g:select name="following" from="${com.grailsinaction.User.list()}" multiple="multiple" optionKey="id" size="5"
              value="${userInstance?.following*.id}" class="many-to-many"/>
</div>
    ...
    <ul class="one-to-many">
        ...
        <li class="add">
            <g:link controller="post" action="create"
                    params="['user.id': userInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'post.label', default: 'Post')])}</g:link>
        </li>
    </ul>
...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'tags', 'error')} ">
    ...
    <ul class="one-to-many">
        ...
        <li class="add">
            <g:link controller="tag" action="create"
                    params="['user.id': userInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'tag.label', default: 'Tag')])}</g:link>
        </li>
    </ul>
